...
function modifyCode(text) {
  ...
  for (const [replacement, code] of Object.entries(replacements)) {
    text = text.replaceAll(
      replacement,
      code[1] ? code[0] : replacement + code[0],
    );
  }
...
(function () {
  ...
  addDump(
    "keyPressedDump",
    "function ([a-zA-Z]*)\\(j\\){return keyPressed\\(j\\)",
  );
  addDump(
    "entitiesDump",
    "this.([a-zA-Z]*).values\\(\\)\\)nt instanceof EntityTNTPrimed",
  );
  addDump(
    "isInvisibleDump",
    "ot.([a-zA-Z]*)\\(\\)\\)&&\\(pt=new ([a-zA-Z]*)\\(new",
  );
  ...
  addDump(
    "syncItemDump",
    "playerControllerMP.([a-zA-Z]*)\\(\\),ClientSocket.sendPacket",
  );
  ...
  addReplacement(
    "if(!nt.canConnect){",
    'nt.errorMessage = nt.errorMessage == "Could not join server. You are connected to a VPN or proxy. Please disconnect from it and refresh the page." ? "You\'re either using a detected VPN server or IP banned for cheating." : nt.errorMessage;',
  );

  // DRAWING SETUP
  addReplacement(
    'ut(this,"glintTexture");',
    `
		ut(this, "vapeTexture");
		ut(this, "v4Texture");
	`,
  ...
  addReplacement(
    ...
    if (ctx$3 && enabledModules["TextGUI"]) {
        ...
        const posX = 15; // X position of the text
        const posY = 17; // Y position of the text

        ctx$3.fillStyle = '#ffffff';
        ctx$3.font = "20px Arial";
        ctx$3.fillText("Arceus", posX, posY); // Render "Arceus" at the top-left
        ...
        // Render the modules on the right
        let modulePosX = 500;  // Adjust this value for positioning the modules to the right
  ...
  addReplacement(
    'updateNameTag(){let$="white",et = 1;',
    "this.entity.team = this.entity.profile.cosmetics.color;",
  );
  addReplacement("connect(_,$=!1,et=!1){", "lastJoined = _;");
  addReplacement(
    'SliderOption("Render Distance ",2,8,3)',
    'SliderOption("Render Distance ",2,64,3)',
    true,
  );
  addReplacement(
    'ClientSocket.on("CPacketDisconnect",$=>{',
    `
  ...
  addReplacement(
    'ClientSocket.on("CPacketMessage",$=>{',
    `
  ...
  addReplacement(
    'ClientSocket.on("CPacketUpdateStatus",$=>{',
    `
  ...
  // REBIND
  addReplacement(
    'bindKeysWithDefaults("b",j=>{',
    'bindKeysWithDefaults("semicolon",j=>{',
    true,
  );
  addReplacement(
    'bindKeysWithDefaults("i",j=>{',
    'bindKeysWithDefaults("apostrophe",j=>{',
    true,
  );

  // SPRINT
  addReplacement(
    'at=keyPressedDump("shift")||touchcontrols.sprinting',
    '||enabledModules["Sprint"]',
  );

  // VELOCITY AND DAMAGEBOOST
  addReplacement(
    '"CPacketEntityVelocity",$=>{const et=j.world.entitiesDump.get($.id);',
    `
		...
		if (player$1 && $.id == player$1.id && enabledModules["DamageBoost"]) {
			if (velocityhori[1] == 0 && velocityvert[1] == 0) return;
			$.motion = new Vector3$1($.motion.x * 0, $.motion.y * 0, $.motion.z * 0);
			const velocity = new Vector3$1(
        player$1.motion.x * damageBoostMultiplier[1],
        player$1.motion.y,
        player$1.motion.z * damageBoostMultiplier[1]
      );
      player$1.motion.set(velocity.x, velocity.y, velocity.z);
		}
	`,
  );
  addReplacement(
  ...
  // KEEPSPRINT
  addReplacement(
    "tt>0&&($.addVelocity(-Math.sin(this.yaw)*tt*.5,.1,-Math.cos(this.yaw)*tt*.5),this.motion.x*=.6,this.motion.z*=.6,this.setSprinting(!1)),",
    `
		if (tt > 0) {
  ...
  // KILLAURA
  addReplacement(
    "else player$1.isBlocking()?",
    "else (player$1.isBlocking() || blocking)?",
    true,
  );
  addReplacement(
    "this.entity.isBlocking()",
    "(this.entity.isBlocking() || this.entity == player$1 && blocking)",
    true,
  );
  addReplacement(
    "const nt={onGround:this.onGround}",
    `, realYaw = sendYaw || this.yaw`,
  );
  addReplacement("this.yaw-this.", "realYaw-this.", true);
  addReplacement("nt.yaw=player.yaw", "nt.yaw=realYaw", true);
  addReplacement(
    "this.lastReportedYawDump=this.yaw,",
    "this.lastReportedYawDump=realYaw,",
    true,
  );
  addReplacement(
    "this.neck.rotation.y=controls$1.yaw",
    "this.neck.rotation.y=(sendYaw||controls$1.yaw)",
    true,
  );

  // NOSLOWDOWN
  addReplacement(
    "const $=this.jumping,et=this.sneak,tt=-.8,rt=this.moveForwardDump<=tt;",
    `
  ...
  addReplacement(
    ...
    "updatePlayerMoveState(),slowdownCheck",
    true,
  );
  ...
  // STEP
  addReplacement(
    "et.y=this.stepHeight;",
    'et.y=(enabledModules["Step"]?Math.max(stepheight[1],this.stepHeight):this.stepHeight);',
    true,
  );

  // WTAP
  addReplacement(
    "this.dead||this.getHealth()<=0)return;",
    `
  ...
  // INVWALK
  addReplacement(
    "keyPressed(j)&&Game.isActive(!1)",
    'keyPressed(j)&&(Game.isActive(!1)||enabledModules["InvWalk"]&&!game.chat.showInput)',
    true,
  );
  ...
  addReplacement(
    "setInterval(()=>this.fixedUpdate(),MSPT)",
    "this.tickLoop=setInterval(()=>this.fixedUpdate(),MSPT)",
    true,
  );

  // PHASE
  addReplacement(
    "calculateXOffset(ft,this.getEntityBoundingBox(),tt.x)",
    'enabledModules["Phase"] ? tt.x : calculateXOffset(ft,this.getEntityBoundingBox(),tt.x)',
    true,
  );
  addReplacement(
    "calculateYOffset(ft,this.getEntityBoundingBox(),tt.y)",
    'enabledModules["Phase"] && keyPressedDump("shift") ? tt.y : calculateYOffset(ft,this.getEntityBoundingBox(),tt.y)',
    true,
  );
  addReplacement(
    "calculateZOffset(ft,this.getEntityBoundingBox(),tt.z)",
    'enabledModules["Phase"] ? tt.z : calculateZOffset(ft,this.getEntityBoundingBox(),tt.z)',
    true,
  );
  addReplacement(
    "pushOutOfBlocks(_,$,et){",
    'if (enabledModules["Phase"]) return;',
  );

  // AUTORESPAWN
  addReplacement(
    "this.game.info.showSignEditor=null,exitPointerLock())",
    `
  ...
  // CHAMS
  addReplacement(
    ")&&(et.mesh.visible=this.shouldRenderEntity(et))",
    `
  ...
  // SKIN
  addReplacement(
    'ClientSocket.on("CPacketSpawnPlayer",$=>{const et=j.world.getPlayerById($.id);',
    `
  ...
  addReplacement(
    'bob:{id:"bob",name:"Bob",tier:0,skinny:!1},',
    'GrandDad:{id:"GrandDad",name:"GrandDad",tier:2,skinny:!1},',
  );
  addReplacement(
    'cloud:{id:"cloud",name:"Cloud",tier:2},',
    'GrandDad:{id:"GrandDad",name:"GrandDad",tier:2},',
  );
  addReplacement(
    "async downloadSkin(_){",
    `
		if (_ == "GrandDad") {
			const $ = skins[_];
  ...
  // LOGIN BYPASS
  addReplacement(
    'new SPacketLoginStart({requestedUuid:localStorage.getItem(REQUESTED_UUID_KEY)??void 0,session:localStorage.getItem(SESSION_TOKEN_KEY)??"",hydration:localStorage.getItem("hydration")??"0",metricsId:localStorage.getItem("metrics_id")??"",clientVersion:VERSION$1})',
    'new SPacketLoginStart({requestedUuid:void 0,session:(enabledModules["AntiBan"] ? "" : (localStorage.getItem(SESSION_TOKEN_KEY) ?? "")),hydration:"0",metricsId:uuid$1(),clientVersion:VERSION$1})',
    true,
  );

  // KEY FIX
  addReplacement(
    "Object.assign(keyMap,_)",
    '; keyMap["Semicolon"] = "semicolon"; keyMap["Apostrophe"] = "apostrophe";',
  );

  // SWING FIX
  addReplacement(
    "player$1.getActiveItemStack().item instanceof",
    "null == ",
    true,
  );
  ...
  // MAIN
  addReplacement(
    'document.addEventListener("contextmenu",j=>j.preventDefault());',
    `
		// my code lol
		(function() {
			class Module {
			...
			// DamageBoost
			let damageBoostMultiplier;
			const damageBoost = new Module("DamageBoost", function() {});
			damageBoostMultiplier = damageBoost.addoption("SpeedMultiplier", Number, 1.5);
			...
			speedvalue = speed.addoption("Speed", Number, 0.4);
  ...
  async function saveVapeConfig(profile) {
    ...
    for (const [name, module] of Object.entries(
      unsafeWindow.globalThis[storeName].modules,
    )) {
      saveList[name] = {
    ...
    GM_setValue(
      "vapeConfig" + (profile ?? unsafeWindow.globalThis[storeName].profile),
      JSON.stringify(saveList),
    );
    GM_setValue(
      "mainVapeConfig",
      JSON.stringify({ profile: unsafeWindow.globalThis[storeName].profile }),
    );
  }

  async function loadVapeConfig(switched) {
    loadedConfig = false;
    const loadedMain = JSON.parse(
      await GM_getValue("mainVapeConfig", "{}"),
    ) ?? { profile: "default" };
    unsafeWindow.globalThis[storeName].profile = switched ?? loadedMain.profile;
    const loaded = JSON.parse(
      await GM_getValue(
        "vapeConfig" + unsafeWindow.globalThis[storeName].profile,
        "{}",
      ),
    );
    if (!loaded) {
  ...
  async function execute(src, oldScript) {
    Object.defineProperty(unsafeWindow.globalThis, storeName, {
      value: {},
      enumerable: false,
    });
